...
const CustomerEditModal = ({ open, onClose, customer }) => {
  ...
  const getSimilarityColor = (score) => {
    if (score >= 90) return "text-status-danger-fg font-bold";
    if (score >= 80) return "text-status-warning-fg font-semibold";
        ...
        <DialogHeader className="px-6 py-4 border-b border-border">
          <DialogTitle className="flex items-center gap-3 text-xl font-semibold">
            <div className="p-2 bg-status-success-bg rounded-lg">
              <Building2 className="w-5 h-5 text-status-success-fg" />
            </div>
            ...
            {similarCustomers.length > 0 && (() => {
              ...
              return (
                <div className="space-y-1.5" data-testid="similarity-warning">
                  {exact && (
                    <div
                      className="flex items-center gap-2 px-3 py-2 rounded-md bg-status-danger-bg border border-status-danger-line text-status-danger-fg text-sm"
                      data-testid="similarity-exact-match"
                    >
                      <AlertTriangle className="w-4 h-4 shrink-0" />
                      <span className="font-semibold truncate">{exact.company_name}</span>
                      <span className="text-xs text-status-danger-fg/80 truncate ml-auto shrink-0">
                        {exact.match_type} · %{exact.similarity_score}
                      </span>
                    </div>
                  )}
                  {others.length > 0 && (
                    <div className="px-3 py-1.5 rounded-md bg-status-warning-bg/70 border border-status-warning-line text-xs">
                      <div className="flex items-center gap-1.5 text-status-warning-fg font-medium mb-0.5">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 mb-2">
                    {formData.tags.map((tag) => (
                      <Badge 
                        key={tag} 
                        className="bg-muted text-foreground pr-1 flex items-center gap-1"
                      >
                        {tag}
                        <button
